# Integration into a QML Application
...
In order to launch the application, you will need to make sure that it finds all required third-party DLLs/SOs.
For this, your options include copying them next to your executable file, configuring the `PATH` environment variable correctly, or using the ImFusion Suite directory as the working directory when executing the application.
Since the ImFusion SDK does not ship any QML/Qt Quick DLLs, you might also need to put the `bin/` directory of the matching Qt version into your `PATH`.
...
The second part defines the source files for the demo application and configures include directories as well as linker input.


### The QML Application Part
The QML application part is rather minimal.
...
The [MainWindow](MainWindow.cpp) class shows the QML scene graph described in [main.qml](main.qml) in a `QQuickView`.
...
The `qtQuickOpenglContextCreated()` callback is called when the Qt Quick scene graph is ready and initializes the ImFusion SDK while sharing the QML OpenGL context.
The `qtQuickOpenglContextAboutToBeDestroyed()` callback properly deinitializes the ImFusion SDK at the right time.
Finally, it provides convenience functions for loading images from disk using the ImFusion IO factories.


### The ImFusionFboView class
The [ImFusionFboView.h](ImFusionFboView.h) and [ImFusionFboView.cpp](ImFusionFboView.cpp) files take care of wrapping the ImFusion rendering stack into the Qt Quick scene graph.
...
The `ImFusionFboView` class implements `QQuickFramebufferObject` which allows for direct integration as a QML object.
It mainly acts as a wrapper around the `ImFusionViewRenderer` and forwards input events to the `DisplayWidgetMulti` so that the user can use the default interaction metaphors to explore the loaded data.
Furthermore, it provides a few example functions illustrating how to change the appearance of the renderer.
